...
# n-gram ration calculation (3 gram and 1 gram)
...
start = time()
print('Parallel processing started...')
est_ratio_list = [pool.apply(calRatio_3, args=(trip_val[(trip_val.time >= startValue + ((i-1)*wndowSize)) & (trip_val.time < startValue + ((i)*wndowSize))].to_numpy(),trip_val.columns)) for i in range(1,int(numWindows-1))]
...
for i in attacks:
    ...
    start = time()
    print('Parallel processing started...')
    attack_freq = [pool.apply(calRatio_3, args=(Attack_dataset[(Attack_dataset.time >= startValue + ((i-1)*wndowSize)) & (Attack_dataset.time < startValue + ((i)*wndowSize))].to_numpy(),Attack_dataset.columns)) for i in range(1,int(numWindows-1))]
